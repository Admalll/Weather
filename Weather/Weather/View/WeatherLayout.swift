...
class WeatherLayout: UICollectionViewLayout {
    ...
    var cachesAttributes = [IndexPath: UICollectionViewLayoutAttributes]()
    ...
    override func prepare() {
        ...
        let smallCellWidt = bigCellWidth / 2
        ...
        for index in 0..<itemsCount {
            ...
            if isBigCell {
                attribute.frame = CGRect(x: 0, y: lastY, width: bigCellWidth, height: cellHeight)
                lastY += cellHeight
            } else {
                attribute.frame = CGRect(x: lastX, y: lastY, width: smallCellWidt, height: cellHeight)
            ...
            cachesAttributes[indexPath] = attribute
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        cachesAttributes.values.filter {
            return rect.intersects($0.frame)
        }
    }

    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        return cachesAttributes[indexPath]
    }
